...
本项目以"专业 RAG 实践"为目标，按 6 个阶段进行了系统性专项优化（仅摸底+修复，不新增功能，唯一新增为 CitationDrawer 的书籍外链按钮）。所有阶段均通过专家团并行评审 + P0/P1 修复闭环。

| 阶段 | 主题 | 关键产出 |
|------|------|----------|
| Phase 0 | P0 阻断修复 | Prisma 建表幂等化（db push）、BGE-reranker 接通 + 3s 超时降级、Toaster 挂载、DOMPurify 同步化（XSS）、pnpm 锁文件治理、pdfkit 迁入 deps |
| Phase 1 | 死代码治理 | 17 个 lib/ 文件转为 re-export shim，单一真源收敛到 server/ + features/ + shared/ |
| Phase 2 | RAG RCR 深化 | docId 去重键、min-max 归一化修复、幻觉校验 fail-closed（check-error / unparseable / 真幻觉三分支）、置信度与热门问题解耦 |
| Phase 3 | 测试与质量门禁 | SSE 契约测试（10 例）、RAG 去重/融合单元测试（6 例）、ATS 回归基线（3 例）、vitest 覆盖率门禁（`pnpm test:coverage`，基线 78/62） |
| Phase 4 | 部署/魔搭优化 | admin 配置持久化到 /mnt/workspace/config（env 覆盖回退 /app/data）、冷启动预热 BGE-M3 + reranker、镜像版本标签（OCI labels）、结构化日志 |
| Phase 5 | 前端稳定性 | consumeSSE 统一（maxRetries=0 防重发）、React.memo 性能优化、error/loading/not-found 路由页、Inter 字体、停止生成按钮、localStorage 批量写（beforeunload + 2s 兜底）、CitationDrawer 书籍外链、WelcomeScreen 场景卡片 |

**质量门禁**：`pnpm ts-check` 0 errors · `pnpm lint` 0 errors · `pnpm test` 1957 passed / 0 failed · `pnpm test:coverage` 门禁通过。

---

## 核心特性

- **流式对话**：基于 SSE 协议，首 token 延迟 < 1s，AI 回复实时增量渲染，支持中途停止生成
- **RAG 检索**：语义检索 + 稀疏检索 + HyDE + 混合排序 4 重保障，docId 去重 + Top-K=5
- **安全门禁**：输入/输出双门禁 + 幻觉校验 fail-closed（校验故障时低置信告警，不阻断已生成内容）
- **状态可见**：检索中 / 生成中状态指示器，降低用户等待焦虑
- **响应式设计**：移动端、平板、桌面端自适应
- **快捷入口**：欢迎页 4 个场景卡片（晋升答辩 / 面试准备 / 能力盘点 / 简历优化），新用户零成本上手
- **书籍外链**：引用面板支持跳转外部读书站点（配置 `NEXT_PUBLIC_BOOK_READER_URL` 启用）
- **侧滑面板**：8 个 Skills 列表 + 清空对话
- **极简风格**：翡翠绿主色 + 纯白背景，Inter 字体，无干扰、聚焦内容
...
| 字体 | next/font/google (Inter) | 自托管，display:swap |
| 表单 | React Hook Form + Zod | 类型安全 |
| LLM | DashScope OpenAI 兼容 (Qwen) | `DASHSCOPE_API_KEY`，仅后端调用 |
| Embedding | BGE-M3 本地 (1024-dim) | 608 向量预置 data/skill-vectors.json |
| Rerank | BGE-reranker-v2-m3 本地 | 懒加载，3s 超时降级 |
| RAG | 语义 + 稀疏 + HyDE → 加权融合 | docId 去重，Top-K=5 |
| 数据库 | Prisma + better-sqlite3 | 双层：4 Prisma 模型 + 7 原生表 |
| 测试 | Vitest 4 | `pnpm test` / `pnpm test:coverage`（门禁 78/62） |
...
| 变量 | 默认 | 作用 |
|------|------|------|
| `REUP_CONFIG_DIR` | `./data`（compose 覆盖为 `/app/data`，ModelScope 为 `/mnt/workspace/config`） | admin 配置（server-config.json）持久化目录 |
| `REUP_PREHEAT` | 生产环境自动启用 | `1` 强制预热 BGE-M3 + reranker；`0` 禁用 |
| `NEXT_PUBLIC_BOOK_READER_URL` | 未设置 | 配置后引用面板显示"阅读相关书籍"外链按钮 |
| `REUP_VERSION` | `latest` | docker-compose 镜像 tag，可 pin 版本 |
