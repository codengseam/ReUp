...
| 工作年限 | `jd-decoder` 输出的 `rigor.years`（如"3 年以上"） | 简历可考证工作年限（按 `resume-evaluator` 的时间线计算，扣除重叠与空窗合理期） | 简历年限 ≥ JD 下限 → 通过；差距 ≤ 1 年且 JD 含"优秀可放宽" → 降为 Important；差距 > 3 年且 JD 无"优秀可放宽" → 淘汰；否则 → Fatal |
...
- 年限淘汰判定：差距 > 3 年且 JD 无"优秀可放宽"说明 → 淘汰，整体分 = 0，无需走后续弹性匹配（见 §3.2 封顶联动）；差距 > 3 年但 JD 含"优秀可放宽" → 仍为 Fatal（封顶 45，继续弹性匹配）；差距 ≤ 3 年的年限缺口按 §1.1 判 Fatal 或降 Important。
...
- 存在年限淘汰（年限差距 > 3 年且 JD 无"优秀可放宽"）→ 整体分 = 0，无需走后续弹性匹配（见 §1.3 硬规则）；此规则优先于"1 项 Fatal 封顶 45"。
