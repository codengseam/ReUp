...
| 现状 | 影响 |
|---|---|
| 4 类 prompt（STAR system / few-shot / ATS / Match Report）硬编码在 [prompts/star.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/prompts/star.ts) / [ats.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/ats.ts) / [matcher.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/matcher.ts) | 改 prompt 需改代码 + 重新部署 |
| 12 份 ATS 评测集 + 跑分逻辑只在 [ats.benchmark.test.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/ats.benchmark.test.ts) | 跑分需 `pnpm test`，无 UI 触发；改 fixture 需改文件 |
...
[src/app/admin/page.tsx](file:///Users/dengxiongshihao/Downloads/reup/src/app/admin/page.tsx) 的 `TAB_CONFIG` 追加：

```ts
{ key: 'resume', label: '简历', icon: FileText },
```

`TabKey` 联合类型同步追加 `'resume'`（[_lib/types.ts](file:///Users/dengxiongshihao/Downloads/reup/src/app/admin/_lib/types.ts)）。

### 3.2 6 个 config key（复用 `/api/admin/config`）

| key | 形态 | 默认（fallback）| 读取方 |
|---|---|---|---|
| `resume.starPrompt` | `string` | 现有 [prompts/star.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/prompts/star.ts) 硬编码 system | star-rewriter + iteration |
| `resume.starFewShot` | `string` (JSON) | 现有 `examples/example-1.json` + `example-2.json` | prompts/star |
| `resume.atsPrompt` | `string` | 现有 [ats.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/ats.ts) 内置 LLM prompt | ats.extractJdKeywords |
| `resume.matchPrompt` | `string` | 现有 [matcher.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/matcher.ts) generatePriorities prompt | matcher.generatePriorities |
...
**API breaking change**：`isPrivacyMode` 从同步变异步。调用方 4 处（[page.tsx L82](file:///Users/dengxiongshihao/Downloads/reup/src/app/resume/page.tsx#L82) / 现有 useEffect 2 处 / 1 处 PrivacyToggle）全部改为 `await` / `useEffect` 内 `void isPrivacyMode().then(set)`。

### 3.5 prompt / config 运行时注入（4 个文件）

| 文件 | 改动 |
|---|---|
| [prompts/star.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/prompts/star.ts) | `buildStarRewritePrompt` 新增 `opts.systemOverride?: string` 入参（覆盖内置 system）；`exampleIds` 已支持 runtime |
| [ats.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/ats.ts) | `extractJdKeywords` 新增 `opts.topK?: number`（默认从 admin-config 拉）；system 段也支持 override |
| [matcher.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/matcher.ts) | `generatePriorities` 新增 `opts.systemOverride?: string` |
| [star-rewriter.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/star-rewriter.ts) | `confidence = min(1, len/confidenceChars)`，chars 来自 admin-config |
| [iteration.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/iteration.ts) | 透传 systemOverride |
...
服务端跑 `extractJdKeywords`（TF 路径，不调 LLM）+ `computeAtsCoverage`，逻辑从 [ats.benchmark.test.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/ats.benchmark.test.ts) 提取到新模块：
...
复用现有 [PromptTab](file:///Users/dengxiongshihao/Downloads/reup/src/app/admin/_components/prompt-tab.tsx) 的 useDebouncedCallback + toast 模式。
...
#### 3.7.2 PromptsCard
- 2×2 grid (md+)，md- 单列
- 每块：`<Label>` + `<Textarea rows=8>` + 右上角 token 估算徽章（复用 [prompts/star.ts](file:///Users/dengxiongshihao/Downloads/reup/src/lib/resume/prompts/star.ts) 的 `estimateTokens`）
