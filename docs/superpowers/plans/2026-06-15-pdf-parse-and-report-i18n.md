...
| `src/lib/resume/parser-pdf.test.ts` | MODIFY | Add 1 e2e test using real `data/示例候选人_软件测试工程师_5年测开经验.pdf` |
...
describe('parseBasicSection pipe-separated rows', () => {
  ...
  it('parses multiple pipe-separated key:value rows on separate lines', () => {
    const doc = parseTextResume(
      '## 个人信息\n' +
      '电话：000-0000-0000 | 邮箱：example@example.com\n' +
      '生日：1998-12 | 性别：男'
    );
    expect(doc.basic.contact?.phone).toBe('000-0000-0000');
    expect(doc.basic.contact?.email).toBe('example@example.com');
...
describe('parsePdfResume real fixture', () => {
  it('extracts basic + experience from 示例候选人 PDF', async () => {
    const pdfPath = resolve(
      __dirname,
      '../../../data/示例候选人_软件测试工程师_5年测开经验.pdf'
    );
...
Manually open the app, upload `data/示例候选人_软件测试工程师_5年测开经验.pdf`, and confirm:
- [ ] `basic.name === '示例候选人'`
