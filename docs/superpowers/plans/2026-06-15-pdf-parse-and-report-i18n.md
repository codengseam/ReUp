...
| `src/lib/resume/parser-pdf.test.ts` | MODIFY | Add 1 e2e test using real `data/[候选人]_resume.pdf` |
...
describe('parseBasicSection pipe-separated rows', () => {
  it('parses pipe-separated phone+email on one line', () => {
    const doc = parseTextResume(
      '## 个人信息\n' +
      '电话：138-0000-0000 | 邮箱：candidate@example.com | 现居城市：北京'
    );
    expect(doc.basic.contact?.phone).toBe('138-0000-0000');
    expect(doc.basic.contact?.email).toBe('candidate@example.com');
  });
  it('parses multiple pipe-separated key:value rows on separate lines', () => {
    const doc = parseTextResume(
      '## 个人信息\n' +
      '电话：138-0000-0000 | 邮箱：candidate@example.com\n' +
      '生日：1998-12 | 性别：男'
    );
    expect(doc.basic.contact?.phone).toBe('138-0000-0000');
    expect(doc.basic.contact?.email).toBe('candidate@example.com');
...
describe('splitSections tail info block', () => {
  it('collects trailing key:value rows into a basic section', () => {
    const doc = parseTextResume(
      ...
      '[候选人]\n' +
      '电话：138-0000-0000 | 邮箱：candidate@example.com | 现居城市：北京\n' +
      '微信：wechat_id | 个人网站：https://example.com'
    );
    expect(doc.basic.name).toBe('[候选人]');
    expect(doc.basic.contact?.phone).toBe('138-0000-0000');
    expect(doc.basic.contact?.email).toBe('candidate@example.com');
  });
});
...
describe('parsePdfResume real fixture', () => {
  it('extracts basic + experience from [候选人] PDF', async () => {
    const pdfPath = resolve(
      __dirname,
      '../../../data/[候选人]_resume.pdf'
    );
    const buf = readFileSync(pdfPath);
    const doc = await parsePdfResume(buf);
    expect(doc.basic.name).toBe('[候选人]');
    expect(doc.basic.contact?.phone).toContain('138-0000');
    expect(doc.basic.contact?.email).toBe('candidate@example.com');
...
Manually open the app, upload `data/[候选人]_resume.pdf`, and confirm:
- [ ] `basic.name === '[候选人]'`
- [ ] `basic.contact.phone` contains `138-0000`
- [ ] `experience.length >= 2`
- [ ] `skills.length >= 5`
- [ ] `education.length >= 1`
