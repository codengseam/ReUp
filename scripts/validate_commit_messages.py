#!/usr/bin/env python3
"""校验指定范围内的 Git 提交信息是否符合 ReUp 中文提交规范。

参考 DreamTale scripts/validate_commit_messages.py, 适配 ReUp 项目。

规范:
  - 标题必须包含中文 (项目约定: 中文优先, 允许英文 type 前缀如 feat/fix/docs)
  - 标题长度 <= 72 字符
  - 正文必须存在且包含中文, 说明"为什么改"和"改了什么"
  - 拒绝默认合并提交信息 (Merge branch / Merge pull request)
...
# 允许的 type 前缀 (Conventional Commits 英文前缀, 后接中文描述)
ALLOWED_PREFIXES = (
...
def validate_message(commit_hash: str, message: str) -> List[str]:
    ...
    # 标题必须包含中文 (允许英文 type 前缀 + 中文描述)
    if not CHINESE_RE.search(subject):
        errors.append(f"标题必须包含中文: '{subject}'")

    # 标题长度 (中文字符按 1 计, 宽松上限 72)
    if len(subject) > 72:
        errors.append(f"标题超过 72 个字符 (当前 {len(subject)}): '{subject}'")
...
def main() -> int:
    ...
    print(f"[validate_commit_messages] {len(commits)} 个提交信息符合规范")
    return 0
