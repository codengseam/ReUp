...
describe('ExportButtons', () => {
  ...
  it('forwards rewritten resume to /api/resume/export when provided', async () => {
    const fetchMock = mockFetchBlob();
    const createObjectURL = vi.fn(() => 'blob:fake-url');
    Object.defineProperty(URL, 'createObjectURL', { configurable: true, value: createObjectURL });
    Object.defineProperty(URL, 'revokeObjectURL', { configurable: true, value: vi.fn() });
    ...
    const rewritten: ResumeDocument = {
      ...resume,
      skills: ['Java', 'Kubernetes', 'Go'],
    };

    render(<ExportButtons resume={resume} starResult={starResult} rewritten={rewritten} />);
    const pdfBtn = screen.getByRole('button', { name: /^PDF$|导出 PDF/i });
    fireEvent.click(pdfBtn);
    ...
    expect(body.rewritten.skills).toEqual(['Java', 'Kubernetes', 'Go']);

    await waitFor(() => {
      expect(clickSpy).toHaveBeenCalled();
    });
    // Anchor download attribute should reflect the rewritten filename
    expect(anchors[0]?.download).toBe('resume-rewritten.pdf');
  });

  it('omits rewritten key from request when not provided', async () => {
    const fetchMock = mockFetchBlob();
    const createObjectURL = vi.fn(() => 'blob:fake-url');
    Object.defineProperty(URL, 'createObjectURL', { configurable: true, value: createObjectURL });
    Object.defineProperty(URL, 'revokeObjectURL', { configurable: true, value: vi.fn() });
    ...
    const body = JSON.parse(String((fetchMock.mock.calls[0] as [string, RequestInit])[1].body));
    expect(body.rewritten).toBeUndefined();
  });

  it('Copy MD uses rewritten skills when provided', async () => {
    const { writeText } = mockClipboard();
    const rewritten: ResumeDocument = {
      ...resume,
      skills: ['Java', 'Kubernetes', 'Go'],
    ...
    const arg = writeText.mock.calls[0]?.[0] as string;
    // The rewritten skills should be present in the markdown
    expect(arg).toContain('Kubernetes');
  });
});
