...
// in the LLM client migration (C5). Document metadata overrides are deferred
// to Phase 1.5. Returns 503 with a stable error envelope so the admin UI can
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  return new Response(
    JSON.stringify({
      ...
      endpoint: '/api/documents/metadata',
    }),
