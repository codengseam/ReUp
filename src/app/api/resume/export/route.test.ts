...
describe('POST /api/resume/export', () => {
  ...
  it('substitutes rewritten content when rewritten is provided (md)', async () => {
    ...
    const res = await POST(jsonRequest({
      ...
      resume: sampleResume,
      rewritten,
      starResult: sampleStar,
    }) as never);
    expect(res.status).toBe(200);
    const body = await res.text();
    // Rewritten skills should be present
    expect(body).toContain('Kubernetes');
    // STAR bullet pattern from the rewritten experience should be present
    expect(body).toContain('*Situation*');
    // Content-Disposition should reflect the rewritten flag
    const cd = res.headers.get('Content-Disposition') ?? '';
    expect(cd).toMatch(/resume-rewritten-.*\.md/);
  });

  it('substitutes rewritten content when rewritten is provided (pdf)', async () => {
    const rewritten: ResumeDocument = {
      ...sampleResume,
      skills: ['Kubernetes'],
    };
    ...
    expect(ct).toMatch(/application\/pdf/);
    const cd = res.headers.get('Content-Disposition') ?? '';
    expect(cd).toMatch(/resume-rewritten-.*\.pdf/);
  });

  it('substitutes rewritten content when rewritten is provided (docx)', async () => {
    const rewritten: ResumeDocument = {
      ...sampleResume,
      skills: ['Kubernetes'],
    };
    ...
    expect(res.status).toBe(200);
    const cd = res.headers.get('Content-Disposition') ?? '';
    expect(cd).toMatch(/resume-rewritten-.*\.docx/);
  });

  it('preserves original basic info (name, contact) when rewritten.basic is sparse', async () => {
    const rewritten: ResumeDocument = {
      ...sampleResume,
      basic: { title: 'Staff 工程师' },
      skills: ['Go'],
    };
    const res = await POST(jsonRequest({
      format: 'md',
    ...
    expect(res.status).toBe(200);
    const body = await res.text();
    // Name from the original should still be in the header
    expect(body).toMatch(/# 张辰/);
  });
});
