...
//
// Loads all fixtures from `src/lib/resume/__fixtures__/ats-bench/*.json`,
// runs the TF-mode `extractJdKeywords` + `computeAtsCoverage` pipeline on
// each, and asserts:
...
//   3. The average coverage across all fixtures is >= 70%
//
// All engines in this benchmark use the TF (no-LLM) path for determinism.
// No LLM is invoked, no network is used.
//
// Fixtures are fully synthetic public data (no real personal data) — see
// `src/lib/resume/__fixtures__/ats-bench/README.md`.
//
...
const FIXTURE_DIR = join(process.cwd(), 'src/lib/resume/__fixtures__/ats-bench');
const TOPK = 20;
const AVG_THRESHOLD = 70;
