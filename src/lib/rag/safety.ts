...
const TOPIC_DETECTION_PROMPT = `你是一个话题分类AI。判断以下用户提问是否属于本知识库支持的话题范围。

本知识库支持的话题由调用方自行定义。不属于支持范围的话题包括：政治、宗教、赌博、毒品、娱乐八卦、游戏攻略等。

请仅回复JSON格式：{"onTopic": true/false, "category": "相关"/"无关", "subCategory": "具体类别"}
  ...
  try {
    ...
    if (topicJsonMatch) {
      ...
      if (!parsed.onTopic) {
        return {
          safe: true,
          reason: `该话题(${parsed.subCategory || parsed.category})超出知识库支持范围，将引导回相关话题`,
          riskLevel: 'medium',
          category: '话题越界',
        };
      }
...
export async function outputGuard(
  ...
  try {
    ...
    const improperPromiseFallback = /保证你一定能|保证成功|保证通过|一定成功|绝对没问题|赔偿|补偿|退款|退款保证/gi;
