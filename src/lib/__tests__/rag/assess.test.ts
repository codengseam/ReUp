...
//
// 框架通用化后：HOT_QUERIES 已清空，isHotQuery 恒返回 false，
// assessConfidence 走纯结果打分路径。
...
describe('assessConfidence - 阶段 4 线性打分', () => {
  it('非热门查询 + 2 条 0.5 分结果 → score=(2/5)*0.5+0.5*0.5=0.45 → medium', () => {
    const r = assessConfidence(mk(0.5, 2), '一段普通的查询 alpha');
...
describe('isHotQuery', () => {
  it('HOT_QUERIES 为空时，任意查询返回 false', () => {
    expect(isHotQuery('一段普通的查询 alpha')).toBe(false);
  });
